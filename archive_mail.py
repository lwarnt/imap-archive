...
from time import localtime as _now
...
def sanitize_string(string) -> "str | None":
    try:  # does not work for every special char, but better than nothing
        string = re.sub(
            r"utf-[0-9][a-z]|iso-[0-9]+-[0-9][a-z]", "", string, flags=re.IGNORECASE
        ).strip()
        return "".join(
            [c for c in string if c in ("@", "-", "_", ".", " ") or c.isalnum()]
        )[:35]
    except:
        return None
        ...
        for mailbox in mailboxes:
            ...
            if not args.all:  # fetch only, if no file for index exists
                if args.zip:
                    ...
                    check_list = list(
                        map(lambda x: x.split(f"{mailbox}/")[-1], check_list)
                    )
                else:
            ...
            chunks = (
                fetch[x : x + args.batch_size]
                for x in range(0, len(fetch), args.batch_size)
            )
            ...
            for batch in batches:
                try:
                    _, _mails = check_response(
                        imap.fetch(bytify(batch), "(RFC822)"), "fetch"
                    )
                    # ('OK', [(b'1 (RFC822 {3642}', b'email header + msg \r\n'), b')'])
                    mails = (m for m in _mails if m not in (b")", b"(", b""))
                    messages = (
                        email.message_from_bytes(mail[1]) for mail in mails
                    ...
                    for msg_id in batch.split(","):
                        ...
                        if not args.dry_run:
                            ...
                            if args.zip:
                                with zf(
                                    archive_path, mode="a", compression=ZIP_LZMA
                                ) as archive:
                                    archive.writestr(
                                        f"{mailbox}/{file_name}", msg_content
                                    )
                            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--port",
        dest="port",
        default=read_config("port") or 993,
        type=str,
    ...
    parser.add_argument(
        ...
        dest="username",
        type=str,
        default=read_config("username") or "",
    ...
    parser.add_argument(
        ...
        dest="password",
        type=str,
        default=read_config("password") or "",
    ...
    mgroup.add_argument(
        ...
        dest="exclude",
        type=str,
        default=read_config("exclude") or "",
    ...
    mgroup.add_argument(
        ...
        dest="include",
        type=str,
        default=read_config("include") or "",
    ...
    parser.add_argument(
        ...
        dest="all",
        action="store_true",
        default=read_config("all") or False,
    ...
    parser.add_argument(
        ...
        dest="batch",
        action="store_true",
        default=read_config("batch") or True,
    ...
    parser.add_argument(
        ...
        dest="batch_size",
        type=int,
        default=read_config("batch_size") or 10,
    ...
    parser.add_argument(
        ...
        dest="zip",
        action="store_true",
        default=read_config("zip") or False,
